...
(function ($) {
    $.fn.redCountdown = function (Options, Callback) {
        ...
        OnlySecends = Settings.onlySeconds;
        ...
        $(this).data("tickInterval", tickInterval);
        $(this).data("ticking", Ticking);
        ...
        function prepareCountdown() {
            ...
            OnlySecondsSum = Settings.end - Settings.now;
            ...
            if (OnlySecends) {
                Element.find(".redCountdownSeconds input").knob($.extend({ width: '100%', displayInput: false, readOnly: true, max: OnlySecondsSum }, Settings.style.secondsElement.gauge));
            }
            else
            {
            ...
            if (!Settings.style.daysElement.isShow) {
                Element.find(".redCountdownDays").hide();
            }

            if (!Settings.style.hoursElement.isShow) {
                Element.find(".redCountdownHours").hide();
            }

            if (!Settings.style.minutesElement.isShow) {
                Element.find(".redCountdownMinutes").hide();
            }
        ...
        function Ticking() {
            ...
            if (OnlySecends) {
                Element.find(".redCountdownSeconds input").val(OnlySecondsSum - SecondsLeft).trigger('change');
            }
            else
            {
    ...
    $.fn.redCountdownPause = function () {
        var tickInterval = $(this).data("tickInterval");

        clearInterval(tickInterval)
    }

    $.fn.redCountdownContinue = function () {
        var ticking = $(this).data("ticking");
        var tickInterval = setInterval(ticking, 1000);
        $(this).data("tickInterval", tickInterval);
    }
