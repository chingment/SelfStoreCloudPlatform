...
namespace Lumos.BLL.Service.AppMobile
{
    ...
    public class CouponService : BaseProvider
    {
        public List<UserCouponModel> My(string operater, string clientUserId, RupCouponMy rup)
        {
            ...
            if (!rup.IsGetHis)
            {
                coupons = CurrentDb.ClientCoupon.Where(m => m.ClientUserId == clientUserId && m.Status == Entity.Enumeration.CouponStatus.WaitUse && m.EndTime > DateTime.Now).ToList();
            }
            else
            {
                coupons = CurrentDb.ClientCoupon.Where(m => m.ClientUserId == clientUserId && (m.Status == Entity.Enumeration.CouponStatus.Used || m.Status == Entity.Enumeration.CouponStatus.Expired) && m.EndTime < DateTime.Now).ToList();
            }
