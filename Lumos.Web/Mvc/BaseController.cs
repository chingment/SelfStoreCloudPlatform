...
namespace Lumos.Web.Mvc
{
    ...
    public abstract class BaseController : Controller
    {
        ...
        #region 公共的方法
        public string ConvertToZTreeJson(object obj, string idField, string pIdField, string nameField, string iconSkin, params string[] isCheckedIds)
        {
            ...
            foreach (var model in (object[])obj)
            {
                ...
                foreach (PropertyInfo p in t1.GetProperties())
                {
                    ...
                    else if (name == pIdField.Trim().ToLower())
                    {
                        ...
                        {
                            Json.Append("\"iconSkin\":\"" + iconSkin + "\" ");
                            Json.Append(",");
                        }
                        else
                        {
                            Json.Append("\"iconSkin\":\"" + iconSkin + "s\" ");
                            Json.Append(",");
                        }
